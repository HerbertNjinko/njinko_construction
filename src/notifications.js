...
function buildAllocationRequestSubmittedAlertBody({
  managerName,
  participantName,
  participantEmail,
  participantCategory,
  ...
  return [
    `Hello ${managerName},`,
    "",
    `${participantName} submitted a project allocation request for ${dealName}.`,
    "",
    `Participant email: ${participantEmail || "Not recorded"}`,
    `Participant type: ${participantCategory}`,
    `Requested amount: $${Number(amount ?? 0).toLocaleString("en-US", { maximumFractionDigits: 2 })}`,
    `Submitted at: ${submittedAt}`,
    "",
...
function buildAllocationRequestReviewedBody({
  fullName,
  dealName,
  amount,
  status,
  ...
  return [
    ...
    "",
    approved
      ? `Your project allocation request for ${dealName} has been approved.`
      : `Your project allocation request for ${dealName} was not approved.`,
    "",
    `Requested amount: $${Number(amount ?? 0).toLocaleString("en-US", { maximumFractionDigits: 2 })}`,
    `Status: ${approved ? "Approved" : "Rejected"}`,
    managerNotes ? `Manager note: ${managerNotes}` : "Manager note: None provided.",
...
function buildUserQuestionSubmittedAlertBody({
  managerName,
  investorName,
  investorEmail,
  subject,
  ...
  return [
    `Hello ${managerName},`,
    "",
    `${investorName} submitted a new portal question.`,
    "",
    `Investor email: ${investorEmail || "Not recorded"}`,
    `Subject: ${subject}`,
    `Submitted at: ${submittedAt}`,
    ...
    questionText,
    "",
    `Respond in the manager portal: ${resolveLoginUrl()}`,
    "",
    `This notice was generated by ${COMPANY_NAME}.`
  ].join("\n");
}

function buildUserQuestionAnsweredBody({
  fullName,
  subject,
  questionText,
  responseText,
  ...
  return [
    `Hello ${fullName},`,
    "",
    `A manager responded to your ${COMPANY_NAME} portal question.`,
    "",
    `Subject: ${subject}`,
    ...
    responseText,
    "",
    `View it in the portal: ${resolveLoginUrl()}`,
    "",
    `If you have questions, contact ${COMPANY_NAME}.`
  ].join("\n");
}
...
export async function sendAllocationRequestSubmittedAlertNotification({
  userId,
  participantId,
  managerName,
  email,
  ...
  return queueAndDeliverNotification({
    ...
    participantId,
    recipientEmail: email,
    subject: `Project allocation request submitted: ${dealName}`,
    bodyText: buildAllocationRequestSubmittedAlertBody({
      managerName,
      participantName,
      participantEmail,
      participantCategory,
...
export async function sendAllocationRequestReviewedNotification({
  userId,
  participantId,
  fullName,
  email,
  ...
  return queueAndDeliverNotification({
    ...
    subject:
      status === "approved"
        ? `Project allocation request approved: ${dealName}`
        : `Project allocation request rejected: ${dealName}`,
    bodyText: buildAllocationRequestReviewedBody({
      fullName,
      dealName,
      amount,
      status,
...
export async function sendUserQuestionSubmittedAlertNotification({
  userId,
  participantId,
  managerName,
  email,
  ...
  return queueAndDeliverNotification({
    ...
    participantId,
    recipientEmail: email,
    subject: `Portal question submitted: ${subject}`,
    bodyText: buildUserQuestionSubmittedAlertBody({
      managerName,
      investorName,
      investorEmail,
      subject,
...
export async function sendUserQuestionAnsweredNotification({
  userId,
  participantId,
  fullName,
  email,
  ...
  return queueAndDeliverNotification({
    ...
    participantId,
    recipientEmail: email,
    subject: `Portal question answered: ${subject}`,
    bodyText: buildUserQuestionAnsweredBody({
      fullName,
      subject,
      questionText,
      responseText,

...
DWOLLA_TERMS_URL=https://...
DWOLLA_PRIVACY_URL=https://...
```
...
The user ACH setup UI includes Dwolla drop-in components for identity upgrade, guided bank linking, and micro-deposit verification, while keeping the portal's manual ACH forms available as a fallback. The drop-ins use `https://cdn.dwolla.com/v3.0.0/dwolla-web.js` and `/api/payments/dwolla/client-token` for scoped client tokens.
